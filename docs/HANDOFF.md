...
- SHA-256：`e2e1c015aca2400d4ba753a32addeb8ac7d30a11deea08c67fed87f896edb52a`
- 解包 EXE ProductVersion：`0.3.2.0`
- 解包 EXE SHA-256：`cecc0ed5644ec13e91b027c2e004b0cc10ea0cecd3de1d8d0108c57e8e8ace48`
- 解包 EXE：PE32+，目标 OS `5.2`
- NSIS 外壳：PE32，目标 OS `4.0`（不是应用兼容性判断对象）
- 运行时：Electron `22.3.27`、Sharp `0.32.6`、PDF.js `2.16.105`
- 签名：未签名
...
- 正式公开分发前建议配置可信代码签名。
- 标准版 `npm audit --omit=dev` 为 0；Win7 遗留依赖审计为 2 个 high（PDF.js、Sharp）。PDF.js 动态执行已通过 `isEvalSupported: false` 缓解；Sharp 0.32 的 libvips 风险仍存在，Win7 包只建议离线处理可信文件。

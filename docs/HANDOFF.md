...
- 2026-08-07 闲鱼销售款 UI 改版（测试 49/49）：按用户要求去掉鼠鼠吉祥物 UI + 删除打赏渠道，改为清爽柔和风（橙色主色 `#ff7a45`、圆角卡片、无吉祥物）。改动：public/index.html（brand-mark ⚡ + 「格式转换，一步到位」标题、拖拽区 📁 + 「把文件拖到这里，或点击选择」、favicon 改 `assets/app-icon.svg`、删除 sponsor-widget 整块）、public/styles.css（全套视觉变量与组件重写，保留全部功能类名）、public/app.js（删除 mouseAssets/setMouseState/mouseStateForConversion 及全部调用点、删除打赏伸缩窗逻辑）、electron-main.js（删除 `--flyingmouse-store` additionalArguments）、preload.js（删除 isStore 字段）、tests/ui-static.test.js 与 tests/mouse-assets.test.js 同步改为断言无鼠鼠/无打赏 + 新图标存在。README 删除「请鼠鼠吃小鱼干」章节；AGENTS.md 更新 UI 描述。构建开关 `--flyingmouse-store` 机制已随打赏一起移除（无保留必要）。
- 2026-08-07 Win7 兼容版（测试 48/48，基于 UI 改版前源码）：用户反馈 Win7 用户报「不是有效的 Win32 应用程序」——主程序 PE 头要求系统 10.0（Electron 43），Win7 内核 6.1 被加载器拒绝。解法：独立构建目录 `D:\34615\飞鼠格式-win7`（未入库，可重建，方法见 AGENTS.md「Win7 兼容版」小节）降级 electron 22.3.27（最后支持 Win7 的版本，需 Win7 x64 SP1）+ sharp 0.32.6 + pdfjs-dist 2.16.105（3.11.x legacy 在 Node 16 崩溃，2.16.105 动态 import 可用）；server.js loadPdfjs 加 `.mjs`→`.js` fallback。验证：win7 副本 npm test 48/48（改版前测试集）、PE 头 OS requirement 5.2、win-unpacked 冒烟启动通过。已上传 GitHub Release v0.2.1（资产 `FlyingMouse.Format-Setup-0.2.1-win7-x64.exe`），README/AGENTS 同步。注意：win7 包基于改版前源码，含鼠鼠 UI + 打赏窗；如需无打赏 win7 版需基于现主线重建。
...
| NSIS 安装包（Win10/11） | `dist\FlyingMouse Format-Setup-0.2.1-x64.exe` | `1DEC82457297EB7275FBAB79155487C2F5550E5A4A5EDC3C748884DDCBB34F69` |
| NSIS 安装包（Win7 兼容版） | `D:\34615\飞鼠格式-win7\dist\FlyingMouse Format-Setup-0.2.1-win7-x64.exe`（GitHub Release v0.2.1 资产名 `FlyingMouse.Format-Setup-0.2.1-win7-x64.exe`） | `4BAF2C070F1E2EFB24E2DFCA9D6E12B688B5ABB7F75143B5E113F4F3DDF45877` |

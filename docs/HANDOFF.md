...
- **双轨策略定调（用户）**：full-version = 母版分支，一切新功能/修复先落满血版；main（公开/商店线）是其阉割派生，**特殊音频格式解锁（ncm/kgg/mgg/mflac/kgma/mmp4/kwm）永不进发布物**，同步只允许 cherry-pick 合规提交。已固化进 AGENTS.md Packaging and release 段。满血版发布通道：本地构建 + GitHub **prerelease**（已发 v0.5.4：https://github.com/LaoFeng-mouse/flyingmouse-format/releases/tag/v0.5.4 ，资产 FlyingMouse.Format-Setup-0.5.4-x64.exe 1,711,988,034B uploaded，sha256 bf9c3fca…，notes=release-notes-054.md 含 053 并入说明）。**未走** tag→CI 路径，原因：两线 docstructure 分叉（CI win32-core 分卷仍是修复前旧引擎 e3278174，装机/发布物是新引擎 8a8f57e——若走 CI 构建反而倒退），main 发布链另有 4 处修复未回流 full-version（探针 bash 化/audit-level=high/资产去空格/已存在跳过）。后续工程决策待定：重打分卷统一引擎（方案B）或建满血专属 ci-engines-v2（方案C）。注：仓库 public，full-version 源码含解锁实现是公开事实，用户知悉。
- **D 盘迁移完成 + 完整性验证（2026-09-03）**：源码仓库 `Documents\飞鼠格式` → `D:\飞鼠格式-src` + junction（git fsck 对象完整、工作树与远端零偏差、junction 老路径全兼容）；满血版整包复制到 `D:\飞鼠格式\FlyingMouse Format`，与 C 盘版逐字节比对 **EXE/app.asar/docengine/docstructure/ffmpeg 全 MATCH、0 大小差异**（C 独有的 1952 文件全是 .bak-20260815 旧引擎垃圾与 app-update.yml，D 盘版更干净）；冒烟启动通过（进程存活 12s），桌面新建「飞鼠格式(满血版).lnk」。**npm test 在 D 盘首跑暴露 2d8045a 的雷**：只同步了 manifest 分卷版、漏配 restore-ci-engines.ps1/测试/release.yml，ci-engine-release.test.js:67 fail——已从 origin/main checkout 三件套配套修复（同时带回 4 处发布链修复：探针 bash 化/audit-level=high/资产全名去空格/已存在跳过 create），修复后全量 539 pass / 0 fail / 5 skip。教训：**manifest/restore/测试/release.yml 是 lockstep 四件套，同步必须整组走**。
- **双轨提醒（同步 release.yml 后生效）**：该 workflow `push: tags v*` + `--draft=false --latest`，满血版线**永不推 v* tag**（会触发 CI 自动发正式版占 Latest）；满血版发布只走本地构建 + 手动 prerelease。
- **遗留**：C 盘旧安装 `C:\Users\34615\飞鼠格式`（4.2G，含 .bak 垃圾）与旧桌面 lnk「FlyingMouse Format.lnk」、桌面两个 2G 级旧包（0.6.4 appx / 满血版 zip 8月旧交付物）待用户确认后清理。
- **D 根散项终清（09-03 第二批，用户点名授权）**：删除 `行程图普吉`(7.3M)/`网络环境部署`(239K)/`文档修复`(已不存在)；`压缩包`(48M)/`文档资料`(33M) 移入 `D:\项目` + 原位 junction；`紫鸟`(477M)/`樱花猫`(415M) 为软件本体留根未动（注册表锚定原则外，用户未点名）。工作副本真身 `D:\飞鼠格式-src` → `D:\项目\飞鼠格式-src`，`D:\飞鼠格式-src` 与 `C:\Users\34615\Documents\飞鼠格式` 均留 junction（Documents 那条暂为双跳链，功能验证通过，终端占用释放后可拉直指 `D:\项目\飞鼠格式-src`）。**D 根 junction 共 22 条全部机械验证 OK**（哨兵文件经老路径可读）。
- **清场完成（2026-09-03 用户确认）**：C 盘旧安装 `C:\Users\34615\飞鼠格式`（4.3G）删除、旧桌面 lnk/开始菜单 lnk 删除、桌面 0.6.4 appx（2.1G）与 8月满血版 zip（2.0G）旧包删除、注册表死卸载项（0.3.7 时代，卸载器已随目录消失）清除并验证无残留。**满血版唯一现役位置 = `D:\软件\飞鼠格式\FlyingMouse Format`**（桌面「飞鼠格式(满血版).lnk」直达）；C 盘可用 172G。同日 D 盘根整理：散件归 `D:\软件` / `D:\项目` 并原位建 junction（含乐不思鼠仓库 → `D:\项目\乐不思鼠`），注册表锚定软件留根未动。
- **bin/pdf2docx/ 旧壳已删**（224MB，08-15 引擎合并前的历史残留：零代码引用、不在 extraResources、不在 CI bundle，经用户确认删除；AGENTS.md 死引用为唯一受害面已修）。
- **提交（full-version，均已推 origin）**：`2a34a1e`（media.js 新增 assertConvertibleToAudio：损坏/读不了的媒体报 MEDIA_INPUT_UNREADABLE「文件无法读取，可能已损坏」，与真无音轨的 MEDIA_NO_AUDIO_TRACK 分开；pdf.js docengine 版式还原失败回退时记 WARN 日志）/ `c121374`（qpdf 进引擎包）/ `6cadf3a`（09-01 交接）/ `2d8045a`（ci-engines-v1.json 同步 main 分卷版：repository 带 owner + parts 布局，full-version 线跑 restore 不再报格式错）/ `648878f`（09-02 交接）/ `cd71b7a`（AGENTS.md 引擎死引用 + release notes 053/054）/ `2196e5b`（054 notes 资产名勘误）。测试：定向 28/28，全量 538 pass / 5 skip / 0 fail。
